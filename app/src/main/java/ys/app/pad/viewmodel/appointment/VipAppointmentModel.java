...
import ys.app.pad.databinding.VipAppointmentBinding;
...
public class VipAppointmentModel extends BaseFragmentViewModel {


    private VipAppointmentFragment mFragment;

    public ObservableField<String> vipName = new ObservableField<>();
    public ObservableField<String> vipPhone = new ObservableField<>();
    public ObservableField<String> vipCardNo = new ObservableField<>();
    ...
    public VipAppointmentModel(VipAppointmentFragment mFragment, VipAppointmentBinding binding,String date) {
    ...
    public void initView() {
        binding.etAppointmentSearch.setOnClickListener(new View.OnClickListener() {
            @Override
        ...
        mRxManager.on(Constants.type_vip_info, new Action1<VipInfo>() {
            @Override
            public void call(VipInfo vipInfo) {
                VipAppointmentModel.this.vipInfo=vipInfo;
    ...
    private void getAnimalHttp() {
        if (vipInfo==null){
            Toast.makeText(mFragment.getActivity(),"请选择会员",Toast.LENGTH_SHORT).show();
            return;
        }
        params.clear();
        params.put("headOfficeId",SpUtil.getHeadOfficeId()+"");
        params.put("userId", vipInfo.getId() + "");
        showRDialog();
    ...
    public void setPet() {
        if (petList==null){
            Toast.makeText(mFragment.getActivity(),"请先选择会员",Toast.LENGTH_SHORT).show();
            return;
        }
        if (petList.size()==0){
            showAddDialog();
    ...
    public void selectFrameTime() {
        ...
        if (TextUtil.isEmpty(appointmentTime.get())){
            Toast.makeText(mFragment.getActivity(), "请先预约时间", Toast.LENGTH_SHORT).show();
            return;
        }
        params.clear();
        params.put("headOfficeId", SpUtil.getHeadOfficeId() + "");
    ...
    public void confirmAppointment() {
        ...
        params.put("vipUserPhone",vipInfo.getPhone());
        params.put("vipUserName",vipInfo.getName());
