...
public class PrintUtil {
     ...
     * @throws IOException
     */
    public PrintUtil(OutputStream outputStream, String encoding)  {
        try {
    ...
    public byte[] getGbk(String stText)  {
        byte[] returnText = new byte[0]; // 必须放在try内才可以
        try {
     ...
     * 对齐0:左对齐，1：居中，2：右对齐
     */
    public void printAlignment(int alignment) {
        try {
            mWriter.write(0x1b);
            mWriter.write(0x61);
    ...
    public void setBold(boolean isBold){
        try {
            mWriter.write(0x1b);
            mWriter.write(0x45);
            if (isBold)mWriter.write(0x01);
            else mWriter.write(0x00);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public void printCustomText(String content,int size) {
    ...
    public void printThreeColumn(String left, String middle, String right) {
        int iNum = 0;
    ...
    public static void printTest(BluetoothSocket bluetoothSocket, Bitmap bitmap) {
        ...
        try {
            ...
            pUtil.printText("时间:2017-05-09 15:50:41");
            pUtil.printLine();
