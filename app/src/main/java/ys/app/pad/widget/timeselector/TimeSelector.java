package ys.app.pad.widget.timeselector;
...
import ys.app.pad.widget.timeselector.Utils.DateUtil;
import ys.app.pad.widget.timeselector.Utils.ScreenUtil;
import ys.app.pad.widget.timeselector.Utils.TextUtil;
import ys.app.pad.widget.timeselector.view.PickerView;
...
public class TimeSelector {
    ...
    private final String FORMAT_YMDHM = "yyyy-MM-dd HH:mm";
     ...
     *
     * @param context
     * @param resultHandler 为选取时间后的回调
     * @param startDate 时间控件的可选起始时间
     * @param endDate 结束时间
     */
    public TimeSelector(Context context, ResultHandler resultHandler, String startDate, String endDate) {
        ...
        startCalendar.setTime(DateUtil.parse(startDate, FORMAT_YMDHM));
        endCalendar.setTime(DateUtil.parse(endDate, FORMAT_YMDHM));
        initDialog();
        initView();
    }
    ...
    private void initView() {
        ...
        tv_select.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (typeValue==1){
                    handler.handle(DateUtil.format(selectedCalender.getTime(), FORMAT_YMD));
                }else{
                    handler.handle(DateUtil.format(selectedCalender.getTime(), FORMAT_YMDHM));
                }
...
//        day_pv.setSelected(0);
    ...
    private void setScrollPosition(){
        ...
        String currentYear = cale.get(Calendar.YEAR)+"";
        String currentMonth = fomatTimeUnit(cale.get(Calendar.MONTH) + 1);

        String currentDay =  fomatTimeUnit(cale.get(Calendar.DATE));
        for (int i=0;i<year.size();i++){
            if (currentYear.equals(year.get(i))){
                year_pv.setSelected(i);
                break;
            }
        }
        for (int i=0;i<month.size();i++){
            if (currentMonth.equals(month.get(i))){
        ...
        for (int i=0;i<day.size();i++){
            if (currentDay.equals(day.get(i))){
                Logger.d("i=="+i);
                day_pv.setSelected(i);
                break;
            }
        }
        Logger.d("year=="+currentYear+"month=="+currentMonth+"day=="+currentDay);
    }
