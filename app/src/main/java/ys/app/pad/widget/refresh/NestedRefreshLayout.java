...
public class NestedRefreshLayout extends ViewGroup
    ...
    public NestedRefreshLayout(Context context, AttributeSet attrs) {
        ...
        animDuration = ta.getInteger(R.styleable.NestedRefreshLayout_animPlayDuration, 100);
        ...
        addOrUpdatePullView(pullViewHeight, pullView != null ? pullView : new RefreshHeaderView(context));
    }
...
//        Logger.d("DaLongState"+pullState+"                  "+deltaY);
            ...
            if (pullState != IPullRefreshView.State.MOVE_REFRESH) {
                if (pullHelper.canTouchUpToRefresh()) {
                    ...
                    if (pullState != IPullRefreshView.State.MOVE_PULL) {
                        ...
                        pullRefreshView.onRefresh(mReFreshChannleId);
                        pullRefreshView.onPullDowning();
                        if (mOnPullDownListener != null) {
                            mOnPullDownListener.onPullDown();
                        }
                    }
                }
            }
    ...
    public void froceRefreshToState(boolean refresh, long delay) {
        final int scrollY = pullHelper.getScroll();
//        Logger.d("DaLongRefresh"+refresh);
        if (refresh) {
...
//        final boolean canFling = canFilngUp || canFilngDown;
    ...
    public void setOnPullHideListener(OnPullHideListener listener) {
        mOnPullHideListener = listener;
    }
    ...
    private final PullAnimationListener resetListener = new PullAnimationListener() {
        ...
        public void end(Animation animation) {
            if (pullHelper.getScroll() == 0) {
                ...
                if (mOnPullHideListener != null) {
                    mOnPullHideListener.onPullHide();
                }
            }
        }
    };
    ...
    public void setReFreshChannleId(String reFreshChannleId) {
        mReFreshChannleId = reFreshChannleId;
    }
